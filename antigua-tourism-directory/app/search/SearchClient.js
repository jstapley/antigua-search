...
export default function SearchClient() {
  ...
  const performSearch = async (searchTerm) => {
    ...
    try {
      ...
      if (filters.parish) {
        q = q.eq('parish_id', filters.parish)
      }

      if (filters.sortBy === 'name') {
        q = q.order('business_name', { ascending: true })
      } else if (filters.sortBy === 'rating') {
        q = q.order('average_rating', { ascending: false, nullsLast: true })
    ...
    <>
      <div className="flex justify-between items-center mb-6">
        ...
        {(filters.category || filters.parish || filters.sortBy !== 'relevance') && (
          <button onClick={clearFilters} className="text-sm text-brand-600 hover:text-brand-700 font-semibold">
            Clear
          </button>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        <select
          ...
          className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-brand-600 focus:outline-none text-sm md:text-base"
        >
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-brand-700 via-brand-600 to-blue-600 py-8 md:py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <form onSubmit={handleSearch} className="max-w-3xl mx-auto">
            <div className="relative flex items-center">
              ...
              />
              <button type="submit" className="absolute right-2 bg-yellow-400 text-brand-900 px-4 md:px-8 py-2 md:py-3 rounded-full font-bold text-sm md:text-lg hover:bg-yellow-300 transition shadow-lg">
                Search
              </button>
            </div>
          </form>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="lg:hidden mb-6">
          <button
            ...
            className="w-full bg-white border-2 border-gray-200 rounded-lg px-4 py-3 font-semibold text-gray-900 flex items-center justify-center gap-2 hover:border-brand-400 transition"
          >
            <span>⚙️</span>
            <span>Filters & Sort</span>
            {(filters.category || filters.parish || filters.sortBy !== 'relevance') && (
              <span className="bg-brand-600 text-white text-xs px-2 py-1 rounded-full">Active</span>
            )}
          </button>
        </div>
          ...
          {mobileFiltersOpen && (
            <div className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setMobileFiltersOpen(false)}>
              <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <FilterControls />
                <button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-brand-600 text-white py-3 rounded-lg font-bold mt-6 hover:bg-brand-700 transition">
                  Apply Filters
                </button>
              </div>
            </div>
          ...
          <div className="flex-1">
            ...
            {!query && !loading && (
              <div className="bg-white rounded-xl border-2 border-gray-200 p-8 md:p-12 text-center">
                ...
                <p className="text-gray-600 mb-8 text-sm md:text-base">Search 200+ businesses across Antigua & Barbuda</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl mx-auto">
                  <Link href="/categories" className="bg-brand-50 hover:bg-brand-100 border-2 border-brand-200 rounded-lg p-6 transition group">
                    <div className="text-3xl mb-2">📋</div>
                    <h4 className="font-bold text-gray-900 mb-1 group-hover:text-brand-600">Browse Categories</h4>
                    <p className="text-sm text-gray-600">Hotels, Restaurants, Tours & more</p>
                  </Link>
                  <Link href="/parishes" className="bg-blue-50 hover:bg-blue-100 border-2 border-blue-200 rounded-lg p-6 transition group">
                    <div className="text-3xl mb-2">📍</div>
                  ...
                  <Link href="/" className="bg-yellow-50 hover:bg-yellow-100 border-2 border-yellow-200 rounded-lg p-6 transition group">
                    <div className="text-3xl mb-2">🏠</div>
            ...
            {!loading && results.length === 0 && query && (
              <div className="bg-white rounded-xl border-2 border-gray-200 p-8 md:p-12 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
                  <button onClick={clearFilters} className="bg-brand-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-brand-700 transition">Clear Filters</button>
                ...
                <div className="border-t border-gray-200 pt-8 mt-8">
                  ...
                  <div className="flex flex-wrap gap-3 justify-center">
                    ...
                    <Link href="/search?q=beaches" className="bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded-full text-sm font-semibold text-gray-700 transition">Beaches</Link>
                    <Link href="/categories" className="bg-brand-100 hover:bg-brand-200 px-4 py-2 rounded-full text-sm font-semibold text-brand-700 transition">All Categories →</Link>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="grid grid-cols-1 gap-4 md:gap-6">
                {results.map((listing) => (
                  <Link
                    ...
                    className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden hover:shadow-xl hover:border-brand-400 transition-all duration-300 group"
                  >
                    <div className="flex flex-col md:flex-row">
                      ...
                      <div className="flex-1 p-4 md:p-6">
                        <div className="flex items-start justify-between mb-2 gap-2">
                          <h3 className="text-lg md:text-2xl font-bold text-gray-900 group-hover:text-brand-600 transition flex-1">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          ...
                          <span className="text-brand-600 font-semibold group-hover:translate-x-1 transition-transform text-sm md:text-base whitespace-nowrap">
                            View Details →
                          </span>
                        </div>
                      </div>
      ...
      <footer className="bg-gray-900 text-white py-8 md:py-12 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-gray-800 pt-6 md:pt-8 text-center">
            <p className="text-gray-400 text-sm">© 2026 AntiguaSearch.com. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
