...
export async function GET(request) {
  ...
  const in29Days = new Date(now); in29Days.setDate(in29Days.getDate() + 29)
  ...
  const sendReminder = async (listing, daysLeft) => {
    if (!listing.email) return
    const renewUrl = `${siteUrl}/listing/${listing.slug}`
    const expiryDate = new Date(listing.featured_until).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })

    const sent = await sendEmail(
      listing.email,
      `⚠️ Your Featured Listing expires in ${daysLeft} days — ${listing.business_name}`,
      `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f9f9f9; border-radius: 0 0 8px 8px;">
          ...
          <p style="color: #555;"><strong>${listing.business_name}</strong> expires on <strong>${expiryDate}</strong>.</p>
          <ul style="color: #555;">
          ...
          <div style="text-align: center; margin-top: 30px;">
            <a href="${renewUrl}" style="background: #318DD0; color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: bold;">Renew Featured Listing — EC$350/year</a>
          </div>
        </div>
      </div>
      `
  ...
  for (const listing of (expiring30 || [])) await sendReminder(listing, 30)
  for (const listing of (expiring7 || [])) await sendReminder(listing, 7)

  return NextResponse.json({ success: true, remindersSent: emailsSent })
}